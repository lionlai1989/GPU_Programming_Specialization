...
__global__ void vecAdd_thread_linear_addressing(int *a, size_t num_elements) {
...
// nvcc thread_linear_addressing.cu && ./a.out 1024 32 16
// nvcc thread_linear_addressing.cu && ./a.out 1030 32 16
int main(int argc, char *argv[]) {
    ...
    std::cout << "Launch " << blocks_per_grid * threads_per_block << " threads. " << "(" << blocks_per_grid << "x"
              << threads_per_block << ")," << "(blocks_per_grid, threads_per_block)" << std::endl;
    // Host is not blocked, so it will not wait for the kernel to finish
    vecAdd_thread_linear_addressing<<<blocks_per_grid, threads_per_block>>>(d_sums_ptr, num_elements);
