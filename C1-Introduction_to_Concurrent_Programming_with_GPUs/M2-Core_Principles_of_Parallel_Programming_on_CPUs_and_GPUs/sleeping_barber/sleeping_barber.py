...
class Customer(threading.Thread):  # Producer Thread
    def __init__(
        self, queue
    ):  # Constructor passes Global Queue (all_customers) to Class
    ...
    def what_customer():
        ...
        customer_rates = {"adult": 16, "senior": 7, "student": 10, "child": 7}
...
class Barber(threading.Thread):  # Consumer Thread
    ...
    def is_empty(
        self,
    ):  # Simple function that checks if there is a customer in the Queue and if so
        if self.queue.empty():
