...
void apply_sobel_filter(Npp8u* d_src, size_t srcStepBytes, int width,
                        int height, Npp16s* d_magnitude, Npp32f* d_direction,
                        int kernel_size) {
  ...
  NppiPoint oAnchor = {1, 1};  // Center of the kernel
  NppiSize oKernelSize = {kernel_size, kernel_size};

  NppStatus status = nppiGradientVectorSobelBorder_8u16s_C1R(
      d_src,                   // pSrc
...
__global__ void non_max_suppression_kernel(Npp16s* suppress, const Npp16s* mag,
                                           const Npp32f* dir, size_t magStep,
                                           size_t angStep, int width,
                                           int height) {
  // 1) Compute pixel coords
  ...
  // 3) Quantize angle to one of 4 directions
  int dx1, dy1, dx2, dy2;
  if ((a < 22.5f) || (a >= 157.5f)) {
    dx1 = -1;
    dy1 = 0;
...
void apply_non_max_suppression(Npp16s* d_suppress, Npp16s* d_magnitude,
                               Npp32f* d_direction, size_t magStep,
                               size_t angStep, int width, int height) {
  // 2D launch parameters :contentReference[oaicite:7]{index=7}
  dim3 block(32, 32);
  dim3 grid((width + block.x - 1) / block.x, (height + block.y - 1) / block.y);

  // Launch kernel
  non_max_suppression_kernel<<<grid, block>>>(
      d_suppress, d_magnitude, d_direction, magStep, angStep, width, height);

  // Check for errors
  cudaError_t cuErr = cudaGetLastError();
  if (cuErr != cudaSuccess) {
    std::cerr << "CUDA kernel launch failed: " << cudaGetErrorString(cuErr)
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
__global__ void hysteresis_thresholding_kernel(Npp8u* edge_map,
                                               const Npp16s* mag,
                                               size_t magStep, int width,
                                               int height, int low_th,
                                               int high_th) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void apply_hysteresis_thresholding(Npp8u* d_edge_map,
                                   const Npp16s* d_suppress_mag, int width,
                                   int height, int low_threshold,
                                   int high_threshold) {
  ...
  hysteresis_thresholding_kernel<<<grid, block>>>(
      d_edge_map, d_suppress_mag, width * sizeof(Npp16s), width, height,
...
cv::Mat process_image(const cv::Mat& input_bgr) {
  ...
  CUDA_CHECK(cudaMemcpy(d_src, input_rgb.data, height * srcStepBytes,
                        cudaMemcpyHostToDevice));

  // Convert RGB to Grayscale
  NppiSize roiSize = {width, height};
  NppStatus status =
      nppiRGBToGray_8u_C3C1R(d_src,          // source pointer
  ...
  CUDA_CHECK(cudaMemcpy(d_kernel, normalized_kernel,
                        kernelSize * kernelSize * sizeof(Npp32s),
                        cudaMemcpyHostToDevice));
  ...
  apply_sobel_filter(d_blur, grayStepBytes, width, height, d_magnitude,
                     d_direction, 3);

  Npp16s* d_suppress_magnitude = nullptr;
  CUDA_CHECK(cudaMalloc(&d_suppress_magnitude, height * magStep));
  apply_non_max_suppression(d_suppress_magnitude, d_magnitude, d_direction,
                            magStep, angStep, width, height);

  Npp8u* d_edge_map = nullptr;
  CUDA_CHECK(cudaMalloc(&d_edge_map, height * width * sizeof(Npp8u)));
  apply_hysteresis_thresholding(d_edge_map, d_suppress_magnitude, width, height,
                                25, 100);

  // convert d_magnitude 16s to 8u
  ...
  CUDA_CHECK(cudaMemcpy(output_gray.data, d_edge_map,
                        height * output_gray.step[0], cudaMemcpyDeviceToHost));
...
int main(int argc, char* argv[]) {
  ...
  cv::VideoWriter writer(
      output_file, cv::VideoWriter::fourcc('a', 'v', 'c', '1'),  // H.264 codec
  ...
  while (cap.read(frame)) {
    ...
    frame_count++;
    if (frame_count % 10 == 0) {
      std::cout << "Processed frame " << frame_count << " of " << total_frames
                << std::endl;
    }
  }
