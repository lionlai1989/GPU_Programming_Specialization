...
void apply_sobel_filter(Npp8u *d_src, size_t srcStepBytes, int width, int height, Npp16s *d_gradient_x,
                        Npp16s *d_gradient_y, Npp16s *d_magnitude, Npp32f *d_direction, int kernel_size,
                        cudaStream_t stream) {
...
class CudaImageProcessor {
  public:
    CudaImageProcessor(int width, int height) : width(width), height(height) {
        // compute buffer sizes
        bgrBytes = size_t(height) * width * 3u; // 3 channels
        grayBytes = size_t(height) * width;
        magBytes = grayBytes * sizeof(Npp16s);
        dirBytes = size_t(height) * width * sizeof(Npp32f);

        // Create two CUDA streams
        CUDA_CHECK(cudaStreamCreate(&stream[0]));

        /**
         * Allocates "bgrBytes_" bytes of host memory that is page-locked and accessible to the device.
         ...
         */
        CUDA_CHECK(cudaHostAlloc(&h_bgr_pinned, bgrBytes, cudaHostAllocDefault));
        CUDA_CHECK(cudaHostAlloc(&h_edge_map_pinned, grayBytes, cudaHostAllocDefault));

        /**
         * Allocates memory with stream ordered semantics for both streams
         */
        // Stream 0 allocations
        CUDA_CHECK(cudaMallocAsync(&d_bgr, bgrBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_gray, grayBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_blur, grayBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_gradient_x, magBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_gradient_y, magBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_magnitude, magBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_direction, dirBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_suppress_mag, magBytes, stream[0]));
        CUDA_CHECK(cudaMallocAsync(&d_edge_map, grayBytes, stream[0]));
    }

    ~CudaImageProcessor() {
        // host
        CUDA_CHECK(cudaFreeHost(h_bgr_pinned));
        CUDA_CHECK(cudaFreeHost(h_edge_map_pinned));

        // device (async free) for stream 0
        CUDA_CHECK(cudaFreeAsync(d_bgr, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_gray, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_blur, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_gradient_x, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_gradient_y, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_magnitude, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_direction, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_suppress_mag, stream[0]));
        CUDA_CHECK(cudaFreeAsync(d_edge_map, stream[0]));

        // cleanup
        CUDA_CHECK(cudaStreamDestroy(stream[0]));
        CUDA_CHECK(cudaDeviceReset()); // Must be called at the end
    }

    void process(const cv::Mat &input_bgr, cv::Mat &output_gray, int stream_id) {
        // Select stream and device pointers based on stream_id
        cudaStream_t &curr_stream = stream[stream_id];
        /**
         * To achieve true asynchronous host–device transfers with cudaMemcpyAsync, it must copy input_bgr into
         * the pinned (page-locked) buffer before launching the async transfer. Simply calling cudaMemcpyAsync directly
         * on input_bgr, which is pageable, will either fall back to a blocking transfer or incur an implicit staging
         * copy under the hood, negating the benefits of pinned memory.
         */
        // std::memcpy(h_bgr_pinned, input_bgr.data, bgrBytes);

        CUDA_CHECK(cudaMemcpyAsync(d_bgr, input_bgr.data, bgrBytes, cudaMemcpyHostToDevice, curr_stream));

        // apply_bgr_to_gray(d_bgr, d_gray, width * 3u, width, width, height, curr_stream);
        apply_bgr_to_gray(d_bgr, d_gray, input_bgr.step[0], width * sizeof(Npp8u), width, height, curr_stream);

        // apply_gaussian_filter(d_gray, d_blur, width, width, width, height, curr_stream);
        apply_gaussian_filter(d_gray, d_blur, width * sizeof(Npp8u), width * sizeof(Npp8u), width, height, curr_stream);

        // FIXME: width??
        // apply_sobel_filter(d_blur, width, width, height, d_gradient_x, d_gradient_y, d_magnitude, d_direction, 3,
        //                    curr_stream);
        apply_sobel_filter(d_blur, width * sizeof(Npp8u), width, height, d_gradient_x, d_gradient_y, d_magnitude,
                           d_direction, 3, curr_stream);

        apply_non_max_suppression(d_suppress_mag, d_magnitude, d_direction, width * sizeof(Npp16s),
                                  width * sizeof(Npp32f), width, height, curr_stream);

        apply_hysteresis_thresholding(d_edge_map, d_suppress_mag, width, height, 25, 100, curr_stream);

        CUDA_CHECK(cudaMemcpyAsync(output_gray.data, d_edge_map, grayBytes, cudaMemcpyDeviceToHost, curr_stream));

        // Wait for the current stream to complete before copying to output
        CUDA_CHECK(cudaStreamSynchronize(curr_stream));
        // std::memcpy(output_gray.data, h_edge_map_pinned, grayBytes);
    }

  private:
    int width, height;
    size_t bgrBytes, grayBytes, magBytes, dirBytes;

    // Use one CUDA stream to overlap memory transfers and kernel launches
    cudaStream_t stream[1];

    // pinned host pointers
    Npp8u *h_bgr_pinned, *h_edge_map_pinned;

    // device pointers for stream
    Npp8u *d_bgr, *d_gray, *d_blur, *d_edge_map;
    Npp16s *d_gradient_x, *d_gradient_y;
    Npp16s *d_magnitude, *d_suppress_mag;
    Npp32f *d_direction;
};
...
void reader_thread(BlockingQueue &queue, cv::VideoCapture &cap) {
    // We'll read into a pinned-host Mat here
    int width = static_cast<int>(cap.get(cv::CAP_PROP_FRAME_WIDTH));
    int height = static_cast<int>(cap.get(cv::CAP_PROP_FRAME_HEIGHT));

    // 1) allocate a pinned HostMem
    cv::cuda::HostMem h_frame(height, width, CV_8UC3, cv::cuda::HostMem::AllocType::PAGE_LOCKED);
    // 2) obtain a Mat header pointing into it
    cv::Mat frame = h_frame.createMatHeader();
...
int main(int argc, char *argv[]) {
    ...
    std::string output_file{"canny_cuda_pinned.mp4"};
    ...
    CudaImageProcessor proc(width, height);

    // Process each frame
    cv::cuda::HostMem h_edge(height, width, CV_8UC1, cv::cuda::HostMem::AllocType::PAGE_LOCKED);
    cv::Mat edge_map = h_edge.createMatHeader();
